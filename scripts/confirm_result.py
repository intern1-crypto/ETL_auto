...
def _fetch_status(client, start_date):
    ...
    return last_order_at, first_planned_at, planned_count, last_mcs_date
...
def main():
    ...
    last_order_at, first_planned_at, planned_count, last_mcs_date = _fetch_status(client, start_date)

    print(f"\n==== 期間: {start_yyyymm[0]}-{start_yyyymm[1]:02d} 〜 "
          f"{end_yyyymm[0]}-{end_yyyymm[1]:02d} ====")

    if last_order_at is not None and last_order_at.date() >= threshold:
        print("来店（最終オーダー日時）: ✅")
    else:
        print(f"来店（最終オーダー日時）: {last_order_at}")

    if first_planned_at is None or first_planned_at.date() >= threshold:
        print("Meetup（参加予定の最初の開始日時）: ✅")
    else:
        print(f"Meetup（参加予定の最初の開始日時）: {first_planned_at}（未確定件数: {planned_count}件）")

    if last_mcs_date is not None and last_mcs_date >= threshold:
        print("MCS（視聴最終日）: ✅")
    else:
        print(f"MCS（視聴最終日）: {last_mcs_date}")
